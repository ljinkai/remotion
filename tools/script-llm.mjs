  ...
  const response = await fetch(`${baseUrl}/chat/completions`, {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: `你是中文口播与字幕编辑。输出严格 JSON。必须依据输入里的 markdownBody / introMarkdown / closingMarkdown 充分改写，覆盖原文要点，禁止只写极短摘要。每个字段用换行分成多行短字幕；行内不要写逗号或句号；每行不超过 ${MAX_SUBTITLE_CHARS} 个汉字；每个案例至少 8 行。英文品牌名与词组（如 Instagram、indie hacker）必须完整留在同一行，禁止中途拆词。`,
        },
        {
          role: "user",
